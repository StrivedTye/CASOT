        ...
        for frame_id in range(len(sequence)):  # tracklet
            ...
            if frame_id == 0:
                ...
                if getattr(self.config, 'attack', False):
                    adv_path = './adv_examples/stnet/' + 'pgd_' + self.config.category_name + '/'
                    os.makedirs(adv_path, exist_ok=True)
                    np.savetxt(adv_path+f'clean_{self.tracklet_count:04.0f}_{frame_id:04d}.txt', data_dict['search_points'][0].detach().cpu().numpy(), fmt='%f')
                    ...
                    np.savetxt(adv_path+f'adv_{self.tracklet_count:04.0f}_{frame_id:04d}.txt', data_dict['search_points'][0].detach().cpu().numpy(), fmt='%f')
