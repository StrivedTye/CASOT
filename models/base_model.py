        ...
        for frame_id in range(len(sequence)):  # tracklet
            ...
            if frame_id == 0:
                ...
                if getattr(self.config, 'attack', False):
                    # self.logger.experiment.add_mesh('search region_ori', data_dict['search_points'],
                    #                                 config_dict={"Size": 10}, global_step=frame_id)
                    adv_path = './adv_examples/' + self.config.net_model + '/' + 'pgd_' + self.config.category_name + '/'
                    os.makedirs(adv_path, exist_ok=True)
                    np.savetxt(adv_path+f'clean_{self.tracklet_count:04.0f}_{frame_id:04d}.txt',
                               data_dict['search_points'][0].detach().cpu().numpy(), fmt='%f')
                    ...
                    # data_dict = self.attack_impluse(data_dict)

                    # self.logger.experiment.add_mesh('search region_adv', data_dict['search_points'],
                    #                                 config_dict={"Size": 10}, global_step=frame_id)
